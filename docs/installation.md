...
1.  **克隆或下载 SDK**
    如果您拥有 Git，可以克隆此仓库：
    ```bash
    git clone https://github.com/Xuanya-Robotics/Bessica-D-SDK.git
    cd Bessica-D-SDK
    ```
...
3.  **硬件连接**
    *   使用 USB 数据线将 Bessica-D 机械臂连接到您的计算机。
    *   确保机械臂已通电。
    *   操作系统通常会自动识别 USB 转串口设备。在 Linux 系统上，设备通常显示为 `/dev/ttyUSB0`、`/dev/ttyUSB1` 等。在 Windows 上，它会显示为 `COMx` (例如 `COM3`), 在MacOS上，它会显示为`cu.usbserial-xxxx` (例如 `cu.usbserial-1440`)
