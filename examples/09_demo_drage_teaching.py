...
def main(args):
    ...
    joints_start = robot.get_joints(arm="right_arm")  # [7_joints]
    start_deg = [np.rad2deg(a) for a in joints_start]

    # 关闭扭矩并拖拽
    input("按 Enter 关闭扭矩，拖拽到目标位置...")
    robot.torque_control(command="off", arm="right_arm")
    input("拖拽完成，按 Enter 继续...")
    robot.torque_control(command="on", arm="right_arm")
    time.sleep(1.0)
    ...
    robot.set_joint_target(target_joints=start_deg, arm="right_arm", joint_format="deg")
    time.sleep(2)
    robot.set_joint_target(target_joints=goal_deg, arm="right_arm", joint_format="deg")
    time.sleep(2)
