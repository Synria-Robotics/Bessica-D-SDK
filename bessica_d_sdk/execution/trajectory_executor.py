...
class _BaseTrajectoryExecutor:
    """Base class with shared trajectory execution logic for dual-arm robots."""
    ...
    def _execute_trajectory(
        self,
        joint_angles: Union[np.ndarray, Dict[str, np.ndarray]],
        ...
        :param joint_angles: For single arm: Array [n_points, n_dof] in radians.
                              For dual arm: Dict with 'left' and 'right' keys, each [n_points, n_dof]
        :param trajectory_times: Optional array of time points [n_points] in seconds.
        :param gripper_values: For single arm: Optional array [n_points] (0-1000).
                              For dual arm: Optional Dict with 'left' and 'right' keys, each [n_points]
        ...
        """
        # Handle dual-arm vs single-arm input format
        if isinstance(joint_angles, dict):
            # Dual-arm mode
            q_left = to_numpy(joint_angles['left'])
            q_right = to_numpy(joint_angles['right'])
            n_points = len(q_left)
            if len(q_right) != n_points:
                raise ValueError(f"Left and right trajectories must have same length")
            is_dual_arm = True
        else:
            # Single-arm mode
            q_single = to_numpy(joint_angles)
            n_points = len(q_single)
            is_dual_arm = False
        ...
        # Handle gripper values
        if gripper_values is not None:
            if isinstance(gripper_values, dict):
                g_left = to_numpy(gripper_values.get('left', None)) if is_dual_arm else None
                g_right = to_numpy(gripper_values.get('right', None)) if is_dual_arm else None
            else:
                g_single = to_numpy(gripper_values)
                if len(g_single) != n_points:
                    raise ValueError(f"gripper_values length ({len(g_single)}) must match joint_angles length ({n_points})")
        else:
            g_left = g_right = g_single = None
        ...
        arm_str = self.arm if not is_dual_arm else "both"
        beauty_print(f"Executing trajectory with {n_points} points ({arm_str})...")
        ...
        for i in range(n_points):
            ...
            if is_dual_arm:
                gripper_val = [
                    int(g_left[i]) if g_left is not None else None,
                    int(g_right[i]) if g_right is not None else None
                ]
            else:
                gripper_val = int(g_single[i]) if g_single is not None else None
            ...
            if is_dual_arm:
                success = self.robot.set_robot_state(
                    target_joints=[q_left[i].tolist(), q_right[i].tolist()],
                ...
                success = self.robot.set_robot_state(
                    target_joints=q_single[i].tolist(),
                    gripper_value=gripper_val,
                    arm=self.arm,
                    joint_format='rad',
                    speed_deg_s=self.speed_deg_s,
...
class JointTrajectoryExecutor(_BaseTrajectoryExecutor):
    """Execute joint space trajectories on the dual-arm robot.
...
class CartesianTrajectoryExecutor(_BaseTrajectoryExecutor):
    """Execute Cartesian space trajectories on the dual-arm robot.
    ...
    def execute(
        ...
        :param joint_angles: For single arm: Array [n_points, n_dof] in radians (from IK).
                              For dual arm: Dict with 'left' and 'right' keys, each [n_points, n_dof]
        ...
        if ik_success_rate is not None and ik_success_rate < min_success_rate:
            ...
            if user_input != 'y':
                ...
                return {
                    ...
                    'executed': 0,
                    'failed': 0,
                    'total': len(joint_angles) if not isinstance(joint_angles, dict) else len(joint_angles['left']),
                    'duration': 0.0,
                    'cancelled': True
                }
